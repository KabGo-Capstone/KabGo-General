...
const ContentComponent: React.FC = () => {
  ...
  const [
    approveDriver,
    {
      data: approve_mutation_data,
      loading: approve_mutation_loading,
      error: approve_mutation_error,
    },
  ] = useMutation(QUERY.APPROVE_DRIVER);
  const [
    disApproveDriver,
    {
      data: disApprove_mutation_data,
      loading: disApprove_mutation_loading,
      error: disApprove_mutation_error,
    },
  ] = useMutation(QUERY.DISAPPROVE_DRIVER);
  const [
    deleteServiceApproval,
    {
      data: delete_mutation_data,
      loading: delete_mutation_loading,
      error: delete_mutation_error,
    },
  ] = useMutation(QUERY.DELETE_SERVICE_APPROVAL);
      ...
      if (dataIndex === "name") {
        return searchedColumn === dataIndex ? (
          <Highlighter
            ...
            autoEscape
            textToHighlight={`${record["supply"]["firstName"] ?? ""} ${
              record["supply"]["lastName"] ?? ""
            }`}
          />
        ) : (
        ...
        <Highlighter
          ...
          textToHighlight={
            record["supply"][dataIndex]
              ? record["supply"][dataIndex].toString()
              : ""
          }
        />
      ) : (
    ...
    },
    {
      ...
      render: (_, record) => (
        <Tag
          ...
          className="!text-sm !p-1.5 !pl-3 !pr-3"
        >
      ...
      render: (_, record) => (
        <div className="!flex gap-2">
          {record.status === "approved" ? (
            ...
            <Button
              style={{ width: "80px" }}
              className="!bg-green-600 !border-transparent !text-white !hover:bg-green-700"
  ...
  const updateData = async () => {
    ...
    axios
      .get(`${process.env.REACT_APP_BACKEND_HOST}/v1/driver/approval`, {
        headers: {
  ...
  const handleDelete = async (record: IDriver) => {
    ...
    axios.delete(`${process.env.REACT_APP_BACKEND_HOST}/v1/driver/approval/` + record?.id, { 
      headers: {
        'Authorization': 'Bearer ' + accessToken
      } 
    })
  ...
  const handleVerify = async (record: IDriver) => {
    ...
    axios.post(`${process.env.REACT_APP_BACKEND_HOST}/v1/driver/approval/approve/` + record?.id, {}, { 
      headers: {
        'Authorization': 'Bearer ' + accessToken
      } 
    })
  ...
  const handleDisapprove = async (record: IDriver) => {
    ...
    axios.patch(`${process.env.REACT_APP_BACKEND_HOST}/v1/driver/approval/disapprove/` + record?.id, {}, { 
      headers: {
        'Authorization': 'Bearer ' + accessToken
      } 
    })
