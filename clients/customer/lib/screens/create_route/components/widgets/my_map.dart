  ...
  void drawRouteDriver() async {
    ...
    markers.add(
      Marker(
        ...
        anchor: const Offset(0.5, 0.5),
        position:
            LatLng(driverAcceptedModel.directions[driverAcceptedModel.directions.length - 1].latitude, driverAcceptedModel.directions[driverAcceptedModel.directions.length - 1].longitude),
        icon: BitmapDescriptor.fromBytes(
          await getBytesFromAsset(
            'lib/assets/images/map_departure_icon.png',
            80,
    ...
    markers.add(
      Marker(
        rotation: double.parse(driverAcceptedModel.rotate.toString()),
        markerId: const MarkerId('driverLocation'),
        anchor: const Offset(0.5, 0.5),
        position: LatLng(driverAcceptedModel.directions[0].latitude,
            driverAcceptedModel.directions[0].longitude),
        icon: bitmapDescriptor!,
      ),
    );
    ...
    PolylinePoints polylinePoints = PolylinePoints();
    List<PointLatLng> result = driverAcceptedModel.directions;
    if (result.isNotEmpty) {
      polylineCoordinates.clear();
      for (var point in result) {
        polylineCoordinates.add(LatLng(point.latitude, point.longitude));
  ...
  Widget build(BuildContext context) {
    ...
    if (mapStep == 'get_current_location') {
      ...
      socketClient!.subscribe('driver-accepted', (dynamic value) {
        ...
        DriverAcceptedModel driver =
            DriverAcceptedModel.fromMap(value['tripInfo']);
        ref.read(driverAcceptProvider.notifier).setDriver(driver);
      ...
      socketClient!.subscribe('driver-moving', (dynamic value) {
        DriverAcceptedModel driver =
            DriverAcceptedModel.fromMap(value['tripInfo']);
        ref.read(driverAcceptProvider.notifier).setDriver(driver);
        drawRouteDriver();
      ...
      socketClient!.subscribe('driver-come', (dynamic value) {
        DriverAcceptedModel driver =
            DriverAcceptedModel.fromMap(value['tripInfo']);
        ref.read(driverAcceptProvider.notifier).setDriver(driver);
        drawRouteDriver();
      ...
      socketClient!.subscribe('trip-finish', (dynamic value) {
