...
class Application {
    ...
    public run(callback: () => void = () => {}): Server {
        ...
        const server: Server = this.app.listen(availablePort, async () => {
            ...
            // clientSocket.init('ws://127.0.0.1:5004')

            // const clientIO = clientSocket.client()

            // clientIO.on('connect', () => {
            //     Logger.info(chalk.green('Client connected to gps service'))
            // })

            // clientIO.on('near-driver', (data) => {
            //     io.customer_sockets[data.customer].socket.emit(
            //         'near-driver',
            //         data.drivers
            //     )
            // })
            // -------------------------------------------------------------------
            // Bổ sung bước lấy từ queue supply-suggesting xuống để lọc ra 5 tài xế gần nhất cho khách hàng tương ứng
            // đã code trong file kafka.ts
            callback()
        })
